...
namespace ERY.Xle
{
    class XleCore
    {
        ...
        public void Run(string[] args)
        {
            ...
            FileManager.ImagePath = new SearchPath("Images");
            FileManager.SoundPath = new SearchPath("Audio");
        ...
        {
            string file = "Maps" + System.IO.Path.DirectorySeparatorChar + inst.mMapList[mapID].Filename;
        ...
        private void RunGame(Player thePlayer)
        {
            ...
            while (Display.CurrentWindow.Closed == false && returnToTitle == false)
            {
                Redraw();
        ...
        public void Draw()
        {
            ...
            Display.FillRect(0, 0, 160, 16, Color.Black);
            Display.FillRect(256, 0, 128, 16, Color.Black);
            Display.FillRect(400, 0, 128, 16, Color.Black);
            Display.FillRect(544, 0, 96, 16, Color.Black);
        ...
        {
            WriteText(px, py, theText, Color.White);
        }
        ...
        {
            if (g.invisible)
                DrawCharacter(anim, vertLine, Color.Gray);
            else if (g.guard)
                DrawCharacter(anim, vertLine, Color.Yellow);
            else
                DrawCharacter(anim, vertLine, Color.White);
        ...
        static void DrawRafts(Rectangle inRect)
        {
            ...
            {
                Player.RaftData raft = player.Rafts[i];
                
                if (map.MapNumber != raft.MapNumber)
                    continue;
        ...
        public static KeyCode WaitForKey(RedrawDelegate redraw, params KeyCode[] keys)
        {
            ...
            do
            {
                ...
                if (Display.CurrentWindow.Closed == true)
                {
                    if (keys.Length > 0)
                        key = keys[0];
                    else
        ...
        static void DrawMenu(SubMenu menu)
        {
            ...
            Display.FillRect(xx, yy, 624 - xx, height, Color.Black);
        ...
        {
            return QuickMenu(items, spaces, 0, Color.White, Color.White);
        }
        public static int QuickMenu(MenuItemList items, int spaces, int value)
        {
            return QuickMenu(items, spaces, value, Color.White, Color.White);
        }
        ...
        {
            return QuickMenu(redraw, items, spaces, 0, Color.White, Color.White);
        }
        public static int QuickMenu(RedrawDelegate redraw, MenuItemList items, int spaces, int value)
        {
            return QuickMenu(redraw, items, spaces, value, Color.White, Color.White);
        }
        ...
        public static int QuickMenu(RedrawDelegate redraw, MenuItemList items, int spaces, int value, Color clrInit, Color clrChanged)
        {
            ...
            } while (key != KeyCode.Return && Display.CurrentWindow.Closed == false);
         ...
         ****************************************************************************/
        [Obsolete("Use a string constructor")]
        static string space(int num)
        {
            string tempstring = new string(' ', num);
        ...
        public static int ChooseNumber(RedrawDelegate redraw, int max)
        {
            ...
            builder.AddText("Enter number by ", Color.White);
            builder.AddText("keyboard", Color.Yellow);
            builder.AddText(" or ", Color.White);
            builder.AddText("joystick", Color.Cyan);
                ...
                if (method == 0)
                {
                    switch (key)
                    {
                        ...
                        case KeyCode.Down:
                            ...
                            g.AddBottom("");
                            g.AddBottom("  Horizontal - Slow change", Color.Cyan);
                            g.AddBottom("  Vertical   - Fast change", Color.Cyan);
                            g.AddBottom("                          - 0 -");
                            ...
                            break;
                        default:
                            g.AddBottom("Keyboard entry-press return when done", Color.Yellow);
                            g.AddBottom("");
                            g.AddBottom("");
                            g.AddBottom("                          - 0 -");
                            method = 1;
