...
ID3DXContext			*g_d3dContext  = NULL;	// Direct 3DX context handle
...
// this function creates the surfaces using D3DX
bool DDCreateSurfaces( bool bFullScreen, int width, int height, int bpp)
{
	...
	if (g_bFullScreen)
	{
		if (S_OK != D3DXCreateContext(D3DX_DEFAULT, D3DX_CONTEXT_FULLSCREEN, g_hWnd,
					width, height, &g_pd3dContext))
			return false;

	}
	else
	{
		if (S_OK != D3DXCreateContext(D3DX_DEFAULT, 0, g_hWnd,
					width, height, &g_pd3dContext))
			return false;

	}

	g_pDD = g_pd3dContext->GetDD();
	g_pDDS = g_pd3dContext->GetPrimary();
	g_pDDSBack = g_pd3dContext->GetBackBuffer(0);

	g_pd3dContext->GetBufferSize (&w, &h);

	backWidth = w;
	backHeight = h;

	//  Create the temp screen
...
bool oldDDCreateSurfaces( bool bFullScreen, int width, int height, int bpp)
{
    ...
    //hr = g_pD3D->CreateDevice( IID_IDirect3DHALDevice, g_pDDSBack,
    //                          &g_pd3dDevice );


	// for now, hard code this to software
    hr = g_pD3D->CreateDevice( IID_IDirect3DRGBDevice, g_pDDSBack,
                              &g_pd3dDevice );
