...
void LotaMap::SetM(int yy, int xx, int val)
{
	...
	{
		//*(m + yy * mapWidth + xx + offset) = val + 65;
...
void LotaMap::Draw( LPDIRECTDRAWSURFACE7 pDDS, int y, int x)
{
	...
	for (j = y - height; j < y + height + 1; j++)
	{
		...
		for (i = x - width; i < x + width + 1; i++)
		{
			...
			if (lastTime + 100 < clock() || g.waterReset == true)
			{
				if (tile == 0 && rnd(1, 1000) < 3 && g.waterReset == false)
				{
					tile = 1;
					SetM(j, i, tile);

				}
				else if ((tile == 1 && rnd(1, 1000) < 45) || (g.waterReset == true && tile == 1))
				{
					tile = 0;
					SetM(j, i, tile);
					//*(m + j * mapWidth + i) = 65;
