...
namespace XleMapEditor
{
	partial class frmRoof : System.Windows.Forms.Form
	{
		...
		private XleMap TheMap { get { return State.TheMap; } }
		public IHasRoofs R
		{
		...
		AgateLib.DisplayLib.DisplayWindow dispWindow;
		AgateLib.DisplayLib.DisplayWindow tilesWindow;
		...
		{
			dispWindow = AgateLib.DisplayLib.DisplayWindow.CreateFromControl(Picture1);
			tilesWindow = AgateLib.DisplayLib.DisplayWindow.CreateFromControl(Picture2);
		...
		public void RoofBlit()
		{
			...
			State.TileSurface.Draw();
			Agate.Rectangle r = new AgateLib.Geometry.Rectangle();
			r.X = (RoofTile % 16) * State.DisplaySize;
			r.Y = (RoofTile / 16) * State.DisplaySize;
			r.Width = State.DisplaySize;
			r.Height = State.DisplaySize;
		...
		private void RoofDraw()
		{
			...
			//			int ddrval;
			AgateLib.Geometry.Rectangle r1 = new AgateLib.Geometry.Rectangle();
			AgateLib.Geometry.Rectangle r2 = new AgateLib.Geometry.Rectangle();
			int tilex;
			int tiley;
			int roofCenterX;
			int xx;
			int yy;
			int roofCenterY;
			AgateLib.Geometry.Point t = new AgateLib.Geometry.Point();
			//			AgateLib.Geometry.Point tile;
			...
			if (chkDrawGround.Checked)
			{
				...
				for (int j = topLeft.Y; j < topLeft.Y + tilesDrawn.Height; j++)
				{
					for (int i = topLeft.X; i < topLeft.X + tilesDrawn.Width; i++)
					{
						...
						r1.X = tilex;
						r1.Width = State.TileSize;
						r1.Y = tiley;
						r1.Height = State.TileSize;

						r2.X = xx * State.DisplaySize;
						r2.Y = yy * State.DisplaySize;
						r2.Width = State.DisplaySize;
						r2.Height = State.DisplaySize;

						State.TileSurface.Draw(r1, r2);
			...
			//r1.X = ( - roofLeftX) * State.DisplaySize;
			//r1.Width = State.TileSize;
			//r1.Y = (-roofTopY) * State.DisplaySize;
			//r1.Width = State.TileSize;

			//Display.DrawRect(r1, Agate.Color.Cyan);
			//Display.DrawRect(new Agate.Rectangle((x2 - roofLeftX) * MainModule.DisplaySize,
			//    (y2 - roofTopY) * MainModule.DisplaySize,
			//    (x2 - roofLeftX + 1) * MainModule.DisplaySize, (y2 - roofTopY) * MainModule.TileSize),
			//    Agate.Color.White);
